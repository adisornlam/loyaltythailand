 ...
 * @author R-D-6
 */
class Users_seller_model extends CI_Model {
    ...
    function get_listall() {
        ...
        $this->datatables->where('users_branch.user_parent', $parent_id);
    ...
    public function set_register_generate($param) {
        $this->load->helper('security');
        $this->load->model('users/Users_model');
        $parent_id = $this->Users_model->get_user_parent($this->ion_auth->get_user_id());
        $str = do_hash($param, 'md5'); // MD5 
        $this->db->insert('users_temp_register', array('user_id' => $this->ion_auth->get_user_id(), 'user_id_ref' => $parent_id, 'keys' => $str));
        return $str;
    }
