            ...
            <div class="tab-pane active" id="tab-1">
                <div class="col-lg-5">
                    <div class="panel-body">
                        <p>สถานะการสั่งซื้อ : <?php echo $order_status; ?> <button type="button" class="btn btn-warning btn-xs btnSucces">รับสินค้าแล้ว</button></p>
                        ...
                        <div class="panel-body">
                            <div class="list-group">
                                <a href="javascript:;" class="list-group-item active">
                                    รายการเปลี่ยนแปลงสถานะ
                                </a>
                                <?php foreach ($order_log->result() as $value) { ?>
                                    <a href="javascript:;" class="list-group-item"><?php echo $value->log_title; ?> <i class="fa fa-clock-o"></i> <?php echo $this->Common_model->get_datethai($value->log_time); ?>
                                        <?php echo ($value->remark ? "<p>หมายเหตุ: " . $value->remark : NULL) . "</p>"; ?>
                                        <?php echo ((isset($value->tf_name) && ( $value->status == 'notified')) ? '<p>ชื่อผู้โอน: ' . $value->tf_name . ' จำนวนเงิน: ' . number_format($value->tf_total) . ' วันเวลา: ' . $value->fdate . ' ' . $value->ftime . ' โอนจาก: ' . $value->from_bank . ' โอนเข้า: ' . $value->to_bank . '</p>' : NULL); ?>
                                    </a>
                                <?php } ?>
                            </div>
                        </div>
...
<script type="text/javascript">
    $('.btnPayment').click(function() {
        load_page();
        var data = {
            url: 'products/backend/order/transfer_add/<?php echo $this->uri->segment(5); ?>',
            title: 'แจ้งชำระเงิน'
        };
        genModal(data);
    });
