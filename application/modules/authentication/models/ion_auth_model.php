...
class Ion_auth_model extends CI_Model {
    ...
    public function fb_login($identity, $password, $remember = FALSE) {
        $this->trigger_events('pre_login');
    ...
    public function get_group_id($param) {

        if ($this->ion_auth->in_group('wholesale')) {
            return $param;
        } elseif ($this->ion_auth->in_group('dealer') || $this->ion_auth->in_group('seller')) {
            $query = $this->db->get_where('users_branch', array('user_id' => $param));
            if ($query->num_rows() > 0) {
                $row = $query->first_row();
                return $row->user_parent;
            } else {
                return 0;
            }
        } else {
    ...
    public function check_dealer($param) {
