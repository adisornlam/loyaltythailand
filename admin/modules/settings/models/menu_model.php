...
class Menu_model extends CI_Model {
    ...
    function listall() {
        ...
        $this->datatables->edit_column('title', '<a href="' . base_url() . index_page() . 'settings/menu/sub/$1">$2</a>', 'id,title');
        $link = "<div class=\"dropdown\">";
        $link .= "<a class=\"dropdown-toggle\" data-toggle=\"dropdown\" href=\"javascript:;\"><span class=\"fa fa-pencil-square-o\"></span ></a>";
        $link .= "<ul class=\"dropdown-menu\" role=\"menu\" aria-labelledby=\"dLabel\">";
        $link .= "<li><a href=\"javascript:;\" rel=\"settings/menu/edit/$1\" class=\"link_dialog\" title=\"Edit Details\">Edit Details</a></li>";
        $link .= "<li><a href=\"javascript:;\" rel=\"settings/result_menu/delete/$1\" class=\"link_dialog delete\" title=\"Delete Menu\">Delete Menu</a></li>";
        $link .= "</ul>";
        $link .= "</div>";
    ...
    public function get_item($id) {
        $this->db->select("menu.*, menu_hierarchy.menu_parent_id");
        $this->db->from("menu");
        $this->db->join("menu_hierarchy","menu_hierarchy.menu_id=menu.id");
    ...
    function add() {
        ...
        if ($this->db->trans_status() === FALSE) {
            ...
            $rdata = array(
                'status' => TRUE,
                'redirect' => 'settings/menu' . ($this->input->post('parent_id', TRUE) ? '/sub/' . $this->input->post('parent_id') : ''),
    ...
    function edit() {
        ...
        if ($this->db->trans_status() === FALSE) {
            ...
            $rdata = array(
                'status' => TRUE,
                'redirect' => 'settings/menu' . ($this->input->post('parent_id')!= 0 ? '/sub/' . $this->input->post('parent_id') : ''),
                'message' => 'Save data success.'
                );
        }
    ...
    public function delete() {
        $this->db->trans_start();
        $this->db->delete('menu', array('id' => $this->uri->segment(4)));
        $this->db->delete('menu_hierarchy', array('menu_id' => $this->uri->segment(4)));
