...
class Course_model extends CI_Model {
    ...
    public function listall() {
        ...
        $this->datatables->select(''
            ...
            . 'course_item.private as private, '
        ...
        if($this->input->post('search_text',true)){
            $this->datatables->like('course_item.title',$this->input->post('search_text'),'both');
        }
        if($this->input->post('private')==1){
            $this->datatables->where('course_item.private',1);
        }
    ...
    function edit_save(){
        ...
        if (!$this->db->update('course_item', $data,array('id'=>$this->input->post('id')))) {
            ...
            if($this->input->post('course_sub_id',true)){
                $this->db->delete('course_item_sub',array('course_item_id'=>$this->input->post('id')));
                foreach ($this->input->post('course_sub_id') as $sub_item) {
                    $this->db->insert('course_item_sub',array('course_item_id'=>$this->input->post('id'),'course_sub_id'=>$sub_item));
                }
            }
            $rdata = array(
                'status' => TRUE,
    ...
    public function get_course_sub_result(){
    ...
    function get_sub_item($id){
        $this->db->select('course_sub_id');
        $this->db->from('course_item_sub');
        $this->db->where('course_item_id',$id);
        $query = $this->db->get();
        $arr = array();
        foreach ($query->result() as $val) {
            $arr[] = $val->course_sub_id;
        }
        return $arr;
    }
}
