...
class Quiz extends MX_Controller {
	...
	public function index() {
		...
		$group = array('admin','owner','teacher');
	...
	public function save() {
		...
		if($this->uri->segment(4)=='add'){
			...
			$rs = $this->Quiz_model->edit_save();
		}else if($this->uri->segment(4)=='delete'){
			$rs = $this->Quiz_model->delete_save();
		}else{
			$rs = array();
		}
	...
	function viewscore(){
		$item = $this->Quiz_model->get_item($this->uri->segment(4));
		$desc_result = $this->Quiz_model->get_quiz_desc_result($item->quiz_id);
		$data = array(
			'title' => 'Quiz View : '.TITLE,
			'title_page' => 'Quiz View',
			'breadcrumbs' => array(
			...
			'item' => $item,
			'desc_result' => $desc_result->result(),
			'course_sub_result' => $this->Quiz_model->get_course_sub_result($this->uri->segment(4)),
			'course_time_result' => $this->Quiz_model->get_quiz_time_result($this->uri->segment(4))
			);
		$group = array('admin','owner','teacher');
		if ($this->ion_auth->in_group($group))
		{
